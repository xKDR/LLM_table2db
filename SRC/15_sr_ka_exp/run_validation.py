...
3. final_sub_head_summary.csv - Sub-Head level data
4. final_detailed_head_summary.csv - Detailed Head level data
...
1. Object Head → Detailed Head
2. Object Head → Sub-Head
3. Object Head → Minor Head
4. Object Head → Major Head

From Detailed Head:
5. Detailed Head → Sub-Head
6. Detailed Head → Minor Head
7. Detailed Head → Major Head

From Sub-Head:
8. Sub-Head → Minor Head
9. Sub-Head → Major Head

From Minor Head:
10. Minor Head → Major Head
"""
...
def validation_detailed_to_sub(detailed_df):
    """
    Validation 2: Detailed Head Totals → Sub Head Total
    Sum all Detailed Head Totals, grouped by hierarchy up to Sub Head
    Compare to Sub Head Totals (V + C combined)
    """
    results = []

    # Get Detailed Head Totals
    detailed_totals = detailed_df[
        (detailed_df['Row_Type'] == 'Total') &
        (detailed_df['Row_Level'] == 'Detailed-Head')
    ].copy()

    # Sum by Major + Minor + Sub (combining V and C)
    detailed_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    detailed_counts = defaultdict(int)

    for _, row in detailed_totals.iterrows():
        major = safe_int(row.get('Major_Head_Code'))
        minor = safe_int(row.get('Minor_Head_Code'))
        sub = safe_int(row.get('Sub_Head_Code'))
        key = (major, minor, sub)
        ...
        for col in FINANCIAL_COLS:
            detailed_sums[key][col] += row[col]
        detailed_counts[key] += 1

    # Get Sub Head Totals
    sub_totals = detailed_df[
        (detailed_df['Row_Type'] == 'Total') &
        (detailed_df['Row_Level'] == 'Sub-Head')
    ].copy()

    # Group sub totals, summing V and C together
    sub_total_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    sub_total_info = {}

    for _, total_row in sub_totals.iterrows():
        major = safe_int(total_row.get('Major_Head_Code'))
        minor = safe_int(total_row.get('Minor_Head_Code'))
        sub = safe_int(total_row.get('Sub_Head_Code'))
        key = (major, minor, sub)
        ...
        for col in FINANCIAL_COLS:
            sub_total_sums[key][col] += total_row[col]

        if key not in sub_total_info:
            sub_total_info[key] = {
                'description': total_row['Description'],
                'page': total_row['Source_Page_Number']
            }
    ...
    for key in detailed_sums:
        if key not in sub_total_sums:
            continue

        major, minor, sub = key
        calc_vals = detailed_sums[key]
        total_vals = sub_total_sums[key]

        result = {
            'Validation': 'Detailed Head Totals → Sub Head Total',
            'Major_Head': major,
            'Minor_Head': minor,
            'Sub_Head': sub,
            'Vote_Charge': 'V+C',
            'Description': sub_total_info[key]['description'],
            'Page': sub_total_info[key]['page'],
            'Detailed_Count': detailed_counts[key],
        }
        ...
        for col in FINANCIAL_COLS:
            ...
            result[f'{col}_DetailedSum'] = round(calc, 2)
            result[f'{col}_SubTotal'] = round(reported, 2)
            result[f'{col}_Diff'] = round(diff, 2)
            result[f'{col}_AbsDiff'] = round(abs_diff, 2)
            result[f'{col}_Accuracy_%'] = round(accuracy, 2)
            result[f'{col}_Match'] = 'PASS' if abs_diff < 0.01 else 'FAIL'
...
def validation_sub_to_minor(sub_df):
    """
    Validation 3: Sub Head Totals → Minor Head Total
    Sum all Sub Head Totals, grouped by Major + Minor
    Compare to Minor Head Totals (V + C combined)
    """
    results = []

    # Get Sub Head Totals
    sub_totals = sub_df[
        (sub_df['Row_Type'] == 'Total') &
        (sub_df['Row_Level'] == 'Sub-Head')
    ].copy()

    # Sum by Major + Minor (combining V and C)
    sub_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    ...
    for _, row in sub_totals.iterrows():
        ...
        for col in FINANCIAL_COLS:
            sub_sums[key][col] += row[col]
        sub_counts[key] += 1

    # Get Minor Head Totals
    minor_totals = sub_df[
        (sub_df['Row_Type'] == 'Total') &
        (sub_df['Row_Level'] == 'Minor-Head')
    ].copy()

    # Group minor totals, summing V and C together
    minor_total_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    minor_total_info = {}
    ...
    for _, total_row in minor_totals.iterrows():
        ...
        for col in FINANCIAL_COLS:
            minor_total_sums[key][col] += total_row[col]

        if key not in minor_total_info:
            minor_total_info[key] = {
                'description': total_row['Description'],
                'page': total_row['Source_Page_Number']
            }
    ...
    # Compare
    for key in sub_sums:
        if key not in minor_total_sums:
            continue

        major, minor = key
        calc_vals = sub_sums[key]
        total_vals = minor_total_sums[key]

        result = {
            'Validation': 'Sub Head Totals → Minor Head Total',
            'Major_Head': major,
            'Minor_Head': minor,
            'Vote_Charge': 'V+C',
            'Description': minor_total_info[key]['description'],
            'Page': minor_total_info[key]['page'],
            'Sub_Count': sub_counts[key],
        }
        ...
        for col in FINANCIAL_COLS:
            ...
            result[f'{col}_SubSum'] = round(calc, 2)
            result[f'{col}_MinorTotal'] = round(reported, 2)
            result[f'{col}_Diff'] = round(diff, 2)
            result[f'{col}_AbsDiff'] = round(abs_diff, 2)
            result[f'{col}_Accuracy_%'] = round(accuracy, 2)
            result[f'{col}_Match'] = 'PASS' if abs_diff < 0.01 else 'FAIL'
...
def validation_minor_to_sub_major(minor_df):
    """
    Validation 4: Minor Head Totals → Sub-Major Head Total
    Sum all Minor Head Totals, grouped by Major + Sub-Major
    Compare to Sub-Major Head Totals (V + C combined)
    """
    results = []

    # Get Minor Head Totals
    minor_totals = minor_df[
        (minor_df['Row_Type'] == 'Total') &
        (minor_df['Row_Level'] == 'Minor-Head')
    ].copy()

    # Sum by Major + Sub-Major (combining V and C)
    minor_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    minor_counts = defaultdict(int)

    for _, row in minor_totals.iterrows():
        major = safe_int(row.get('Major_Head_Code'))
        sub_major = safe_int(row.get('Sub_Major_Head_Code'))
        key = (major, sub_major)

        for col in FINANCIAL_COLS:
            minor_sums[key][col] += row[col]
        minor_counts[key] += 1

    # Get Sub-Major Head Totals
    sub_major_totals = minor_df[
        (minor_df['Row_Type'] == 'Total') &
        (minor_df['Row_Level'] == 'Sub-Major-Head')
    ].copy()

    # Group sub-major totals, summing V and C together
    sub_major_total_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    sub_major_total_info = {}

    for _, total_row in sub_major_totals.iterrows():
        major = safe_int(total_row.get('Major_Head_Code'))
        sub_major = safe_int(total_row.get('Sub_Major_Head_Code'))
        key = (major, sub_major)

        for col in FINANCIAL_COLS:
            sub_major_total_sums[key][col] += total_row[col]

        if key not in sub_major_total_info:
            sub_major_total_info[key] = {
                'description': total_row['Description'],
                'page': total_row['Source_Page_Number']
            }

    # Compare
    for key in minor_sums:
        if key not in sub_major_total_sums:
            continue

        major, sub_major = key
        calc_vals = minor_sums[key]
        total_vals = sub_major_total_sums[key]

        result = {
            'Validation': 'Minor Head Totals → Sub-Major Head Total',
            'Major_Head': major,
            'Sub_Major_Head': sub_major,
            'Vote_Charge': 'V+C',
            'Description': sub_major_total_info[key]['description'],
            'Page': sub_major_total_info[key]['page'],
            'Minor_Count': minor_counts[key],
        }
        ...
        for col in FINANCIAL_COLS:
            ...
            result[f'{col}_MinorSum'] = round(calc, 2)
            result[f'{col}_SubMajorTotal'] = round(reported, 2)
            result[f'{col}_Diff'] = round(diff, 2)
            result[f'{col}_AbsDiff'] = round(abs_diff, 2)
            result[f'{col}_Accuracy_%'] = round(accuracy, 2)
            result[f'{col}_Match'] = 'PASS' if abs_diff < 0.01 else 'FAIL'
...
def validation_sub_major_to_major(sub_major_df):
    """
    Validation 5: Sub-Major Head Totals → Major Head Total
    Sum all Sub-Major Head Totals, grouped by Major
    Compare to Major Head Totals (excluding 'TOTAL V+C')
    """
    results = []

    # Get Sub-Major Head Totals
    sub_major_totals = sub_major_df[
        (sub_major_df['Row_Type'] == 'Total') &
        (sub_major_df['Row_Level'] == 'Sub-Major-Head')
    ].copy()

    # Sum by Major (combining V and C)
    sub_major_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    sub_major_counts = defaultdict(int)

    for _, row in sub_major_totals.iterrows():
        major = safe_int(row.get('Major_Head_Code'))

        for col in FINANCIAL_COLS:
            sub_major_sums[major][col] += row[col]
        sub_major_counts[major] += 1

    # Get Major Head Totals (excluding 'TOTAL V+C')
    major_totals = sub_major_df[
        (sub_major_df['Row_Type'] == 'Total') &
        (sub_major_df['Row_Level'] == 'Major-Head') &
        (~sub_major_df['Description'].str.contains('TOTAL V\\+C', na=False))
    ].copy()

    for _, total_row in major_totals.iterrows():
        major = safe_int(total_row.get('Major_Head_Code'))

        if major not in sub_major_sums:
            continue

        calc_vals = sub_major_sums[major]

        result = {
            'Validation': 'Sub-Major Head Totals → Major Head Total',
            'Major_Head': major,
            'Vote_Charge': 'V+C',
            'Description': total_row['Description'],
            'Page': total_row['Source_Page_Number'],
            'SubMajor_Count': sub_major_counts[major],
        }
        ...
        for col in FINANCIAL_COLS:
            calc = calc_vals[col]
            reported = total_row[col]
            diff = calc - reported
            abs_diff = abs(diff)
            accuracy = calculate_accuracy_percentage(calc, reported)

            result[f'{col}_SubMajorSum'] = round(calc, 2)
            result[f'{col}_MajorTotal'] = round(reported, 2)
            result[f'{col}_Diff'] = round(diff, 2)
            result[f'{col}_AbsDiff'] = round(abs_diff, 2)
            result[f'{col}_Accuracy_%'] = round(accuracy, 2)
            result[f'{col}_Match'] = 'PASS' if abs_diff < 0.01 else 'FAIL'
...
def validation_object_to_sub(object_df):
    """Validation 2: Object Head Data → Sub-Head Total"""
    results = []
    ...
    for _, row in object_data.iterrows():
        ...
        if major == 0 and minor == 0 and sub == 0:
            continue
    ...
    sub_totals = object_df[
        (object_df['Row_Type'] == 'Total') &
        (object_df['Row_Level'] == 'Sub-Head')
    ].copy()
    ...
    for key in object_sums:
        ...
        result = {
            'Validation': 'Object Head → Sub-Head',
            'Major_Head': major,
            'Minor_Head': minor,
            'Sub_Head': sub,
            'Description': sub_total_info[key]['description'],
...
def validation_object_to_minor(object_df):
    """Validation 3: Object Head Data → Minor Head Total"""
    results = []
    ...
    for key in object_sums:
        ...
        result = {
            'Validation': 'Object Head → Minor Head',
            'Major_Head': major,
            'Minor_Head': minor,
            'Description': minor_total_info[key]['description'],
            'Page': minor_total_info[key]['page'],
...
def validation_object_to_major(object_df):
    """Validation 4: Object Head Data → Major Head Total"""
    results = []
    ...
    major_totals = object_df[
        (object_df['Row_Type'] == 'Total') &
        (object_df['Row_Level'] == 'Major-Head')
    ].copy()

    major_total_sums = defaultdict(lambda: {col: 0 for col in FINANCIAL_COLS})
    major_total_info = {}
    ...
    for _, total_row in major_totals.iterrows():
        ...
        key = (major,)
        for col in FINANCIAL_COLS:
            major_total_sums[key][col] += total_row[col]

        if key not in major_total_info:
            major_total_info[key] = {
                'description': total_row['Description'],
                'page': total_row['Source_Page_Number']
            }

    for key in object_sums:
        if key not in major_total_sums:
            continue

        major, = key
        calc_vals = object_sums[key]
        total_vals = major_total_sums[key]

        result = {
            'Validation': 'Object Head → Major Head',
            'Major_Head': major,
            'Description': major_total_info[key]['description'],
            'Page': major_total_info[key]['page'],
            'Object_Count': object_counts[key],
        }
...
def validation_detailed_to_minor(detailed_df):
    """Validation 6: Detailed Head Totals → Minor Head Total"""
    results = []
    ...
    minor_totals = detailed_df[
        (detailed_df['Row_Type'] == 'Total') &
        (detailed_df['Row_Level'] == 'Minor-Head')
    ].copy()
    ...
    for key in detailed_sums:
        ...
        result = {
            'Validation': 'Detailed Head → Minor Head',
            'Major_Head': major,
            'Minor_Head': minor,
            'Description': minor_total_info[key]['description'],
            'Page': minor_total_info[key]['page'],
...
def validation_detailed_to_major(detailed_df):
    """Validation 7: Detailed Head Totals → Major Head Total"""
    results = []
    ...
    major_totals = detailed_df[
        (detailed_df['Row_Type'] == 'Total') &
        (detailed_df['Row_Level'] == 'Major-Head')
    ].copy()
    ...
    for key in detailed_sums:
        ...
        result = {
            'Validation': 'Detailed Head → Major Head',
            'Major_Head': major,
            'Description': major_total_info[key]['description'],
            'Page': major_total_info[key]['page'],
            'Detailed_Count': detailed_counts[key],
...
def validation_sub_to_major(sub_df):
    """Validation 9: Sub-Head Totals → Major Head Total"""
    results = []
    ...
    for key in sub_sums:
        ...
        result = {
            'Validation': 'Sub-Head → Major Head',
            'Major_Head': major,
            'Description': major_total_info[key]['description'],
            'Page': major_total_info[key]['page'],
            'Sub_Count': sub_counts[key],
...
def validation_minor_to_major(minor_df):
    """Validation 10: Minor Head Totals → Major Head Total"""
    results = []
    ...
    major_totals = minor_df[
        (minor_df['Row_Type'] == 'Total') &
        (minor_df['Row_Level'] == 'Major-Head')
    ].copy()
    ...
    for key in minor_sums:
        ...
        result = {
            'Validation': 'Minor Head → Major Head',
            'Major_Head': major,
            'Description': major_total_info[key]['description'],
            'Page': major_total_info[key]['page'],
            'Minor_Count': minor_counts[key],
...
def main(output_dir=None, output_prefix='validation'):
    ...
    if 'object_head' in loaded_dfs:
        ...
        print("   → Validation 2: Object Head → Sub-Head...")
        val2 = validation_object_to_sub(loaded_dfs['object_head'])
        validations.append(val2)
        val_names.append('Object → Sub')
        val_sources.append('object_head_summary.csv')

        print("   → Validation 3: Object Head → Minor Head...")
        val3 = validation_object_to_minor(loaded_dfs['object_head'])
        validations.append(val3)
        val_names.append('Object → Minor')
        val_sources.append('object_head_summary.csv')

        print("   → Validation 4: Object Head → Major Head...")
        val4 = validation_object_to_major(loaded_dfs['object_head'])
        validations.append(val4)
        val_names.append('Object → Major')
        val_sources.append('object_head_summary.csv')

    # === FROM DETAILED HEAD ===
    if 'detailed_head' in loaded_dfs:
        print("\n   From Detailed Head:")
        print("   → Validation 5: Detailed Head → Sub-Head...")
        val5 = validation_detailed_to_sub(loaded_dfs['detailed_head'])
        validations.append(val5)
        val_names.append('Detailed → Sub')
        val_sources.append('detailed_head_summary.csv')

        print("   → Validation 6: Detailed Head → Minor Head...")
        val6 = validation_detailed_to_minor(loaded_dfs['detailed_head'])
        validations.append(val6)
        val_names.append('Detailed → Minor')
        val_sources.append('detailed_head_summary.csv')

        print("   → Validation 7: Detailed Head → Major Head...")
        val7 = validation_detailed_to_major(loaded_dfs['detailed_head'])
        validations.append(val7)
        val_names.append('Detailed → Major')
        val_sources.append('detailed_head_summary.csv')

    # === FROM SUB-HEAD ===
    if 'sub_head' in loaded_dfs:
        print("\n   From Sub-Head:")
        print("   → Validation 8: Sub-Head → Minor Head...")
        val8 = validation_sub_to_minor(loaded_dfs['sub_head'])
        validations.append(val8)
        val_names.append('Sub → Minor')
        val_sources.append('sub_head_summary.csv')

        print("   → Validation 9: Sub-Head → Major Head...")
        val9 = validation_sub_to_major(loaded_dfs['sub_head'])
        validations.append(val9)
        val_names.append('Sub → Major')
        val_sources.append('sub_head_summary.csv')

    # === FROM MINOR HEAD ===
    if 'minor_head' in loaded_dfs:
        print("\n   From Minor Head:")
        print("   → Validation 10: Minor Head → Major Head...")
        val10 = validation_minor_to_major(loaded_dfs['minor_head'])
        validations.append(val10)
        val_names.append('Minor → Major')
        val_sources.append('minor_head_summary.csv')
    ...
    for i, (val_df, val_name) in enumerate(zip(validations, val_names), 1):
        output_path = output_dir / f'{output_prefix}_{i}_{val_name.lower().replace(" → ", "_to_").replace(" ", "_")}.csv'
