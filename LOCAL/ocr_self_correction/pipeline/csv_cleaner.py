...
@dataclass
class Issue:
    """Represents a single validation or cleaning issue found in a row.

    Attributes:
        row_number: The 1-indexed row number where the issue was found.
        column: The name of the column where the issue was found, or "ALL".
        message: A descriptive message explaining the issue.
        code: A short, machine-readable code for the issue type.
        fixed: Whether the issue was automatically corrected.
        severity: The severity level of the issue (e.g., "error", "warning").
    """
...
class RowResult:
    ...
    Attributes:
        ...
        row: The processed row as a list of strings.
        changed: Whether any changes were made to the row.
        issues: A list of Issue objects found during processing.
    """
...
@dataclass
class FileReport:
    """Consolidates processing results and statistics for a single file.

    Attributes:
        input_file: Path to the input CSV file.
        output_file: Path where the cleaned CSV will be saved.
        total_rows: Total number of rows processed.
        cleaned_rows: Total number of rows successfully cleaned.
        rows_with_changes: Number of rows that were modified during cleaning.
        issues: A comprehensive list of all issues found in the file.
        header_replaced: Whether the original header was replaced with the schema header.
    """
    ...
    def add_row(self, row_result: RowResult) -> None:
        """Updates file statistics with the result of a single row's processing."""
        self.total_rows += 1
        self.cleaned_rows += 1
    ...
    def issues_by_row(self) -> Dict[int, List[Issue]]:
        """Groups all issues in the file by row number."""
    ...
    def issue_counts_by_code(self) -> Dict[str, int]:
        """Calculates occurrences of each issue type (code) in the file."""
        counts: Dict[str, int] = {}
        for issue in self.issues:
    ...
    def rows_with_errors_uncorrected(self) -> int:
        """Counts rows with at least one error that could not be fixed."""
        issues_by_row = self.issues_by_row()
        uncorrected_rows = 0
        for row_issues in issues_by_row.values():
    ...
    def warnings_count(self) -> int:
        """Returns the total number of warnings (non-error issues) found."""
        return sum(1 for issue in self.issues if issue.severity == "warning")

    @property
    def issue_count(self) -> int:
        """Returns the total number of issues found in the file."""
        return len(self.issues)


class CleaningLogger:
    """Collects per-run logs and writes them to disk in various formats."""

    def __init__(self, base_dir: Path):
        """Initializes the logger with a base directory for output files."""
    ...
    def append_text(self, line: str = "") -> None:
        """Appends a line of text to the text-based report."""
...
def pad_code(value: str, width: int) -> str:
    """Pads a code string with leading zeros to a specific width.

    Args:
        value: The code string to pad.
        width: The target width for the padded code.

    Returns:
        The padded code string. If the value contains non-numeric characters,
        they are preserved but padding is only applied to digit-only portions.
    """
    if not value:
        return ""
...
def clean_financial_value(value: str) -> str:
    """Normalises financial numeric strings, removing commas and stray characters.

    Args:
        value: The raw financial string to clean.

    Returns:
        A cleaned numeric string that can be safely converted to a float/int.
        Returns an empty string if the value is non-numeric or only contains
        filler markers (e.g., "...").
    """
    if not value:
        return ""
...
class HierarchyContext:
    """Tracks current budget hierarchy state to support code inheritance and inference.

    As rows are processed, this context maintains the most recently seen codes
    at each level (Major, Sub-Major, etc.) to fill in gaps in subsequent rows.
    """
    ...
    def __init__(
        ...
        """Initializes the context with schema mapping and hierarchy rules."""
    ...
    def inherit_codes(self, row: List[str]) -> bool:
        """Fills empty code fields in a row using previously seen hierarchy codes."""
        changed = False
    ...
    def update(self, row: Sequence[str]) -> None:
        """Updates the context state with any non-empty codes present in the current row."""
        for field in self.code_fields:
            idx = self.column_index.get(field)
            if idx is None or idx >= len(row):
                continue
    ...
    def infer_level(self) -> str:
        """Infers the budget hierarchy level based on the deepest available code."""
...
class RowProcessor:
    """Orchestrates the cleaning and validation of individual budget rows."""

    def __init__(
        self, schema_name: str, schema: Sequence[str], financial_columns: List[str]
    ):
        """Initializes the processor with specific schema rules."""
    ...
    def create_context(self) -> HierarchyContext:
        """Creates a new HierarchyContext compatible with this processor's schema."""
    ...
    def process(
        ...
        """Executes the full cleaning and validation pipeline for a row.

        Args:
            raw_row: The raw list of strings from the CSV.
            row_number: The 1-indexed row number.
            context: An optional HierarchyContext for code inheritance.

        Returns:
            A RowResult object containing the cleaned row and any issues found.
        """
        ...
        return RowResult(
            row_number=row_number, row=working, changed=changed, issues=issues
        )
    ...
    def _align_columns(
        ...
        if len(working) < self.expected_cols:
            ...
            insert_pos = self._find_missing_field_position(working, deficit)
            if insert_pos is not None:
                for _ in range(deficit):
                    working.insert(insert_pos, "")
                changed = True
                message = (
                    f"Row had {original_len} columns, expected {self.expected_cols}. "
                    f"Inserted {deficit} empty field(s) at position {insert_pos}."
                )
            else:
    ...
    # Hierarchy code/name pairs in schema order (shallowest to deepest).
    _HIERARCHY_PAIRS: Tuple[Tuple[str, str], ...] = (
        ("Sub_Major_Head_Code", "Sub_Major_Head_Name"),
        ("Minor_Head_Code", "Minor_Head_Name"),
        ("Sub_Head_Code", "Sub_Head_Name"),
        ("Detailed_Head_Code", "Detailed_Head_Name"),
        ("Object_Head_Code", "Object_Head_Description"),
    )

    def _find_missing_field_position(
        self, row: List[str], deficit: int
    ) -> Optional[int]:
        """Locate where fields are missing by finding the Row_Type/Row_Level anchor.

        When the LLM omits hierarchy fields (e.g. Object_Head_Code for a
        Minor-Head total), the tail section (Full_Account_Code onwards) shifts
        left.  By scanning for the recognisable Row_Type + Row_Level pair we
        can detect the actual tail position and insert empty fields at the
        correct place rather than padding at the end.

        For single-field deficits we additionally try to pinpoint the exact
        missing Name/Description column in the hierarchy section by looking
        for a code-like value sitting in a name position.
        """
        if self.row_type_idx is None or self.row_level_idx is None:
            return None

        # --- Step 1: find the tail anchor (Row_Type + Row_Level pair) ---
        tail_insert: Optional[int] = None
        for i in range(len(row) - 1, 0, -1):
            level_candidate = row[i].strip().lower()
            if level_candidate not in ROW_LEVEL_CANONICAL:
                continue
            type_candidate = row[i - 1].strip().lower()
            if type_candidate not in ROW_TYPE_CANONICAL:
                continue

            actual_row_type_pos = i - 1
            shift = self.row_type_idx - actual_row_type_pos
            if shift != deficit or shift <= 0:
                continue

            tail_offset = self.row_type_idx - self.column_index.get(
                "Full_Account_Code", self.row_type_idx
            )
            tail_insert = actual_row_type_pos - tail_offset
            break

        if tail_insert is None or tail_insert <= 0:
            return None

        # --- Step 2: for deficit == 1, try to find the exact missing field ---
        if deficit == 1:
            exact = self._find_missing_hierarchy_name(row, tail_insert)
            if exact is not None:
                return exact

        return tail_insert

    def _find_missing_hierarchy_name(
        self, row: List[str], tail_insert: int
    ) -> Optional[int]:
        """For a single missing field, detect which Name column was omitted.

        Hierarchy columns come in (Code, Name) pairs.  When the LLM drops a
        Name field, the next Code value (short numeric) slides into the Name
        position.  Scanning the pairs lets us pinpoint where to insert.
        """
        for code_col, name_col in self._HIERARCHY_PAIRS:
            name_idx = self.column_index.get(name_col)
            if name_idx is None or name_idx >= len(row) or name_idx >= tail_insert:
                continue
            value = row[name_idx].strip()
            if not value:
                continue
            # A name position holding a short numeric looks like a shifted code.
            if value.replace(".", "", 1).isdigit() and len(value) <= 4:
                return name_idx
        return None
...
class CSVFileProcessor:
    ...
    def __init__(self, schema_name: str, schema: Sequence[str], financial_columns: List[str]):
        self.schema_name = schema_name
        self.schema = list(schema)
        self.row_processor = RowProcessor(schema_name, schema, financial_columns)
    ...
    def process_file(
        ...
        # Detect whether the first row is a real header or a data row.
        # A real header will contain recognisable column names; a data row
        # typically starts with a numeric page number.
        first_row = rows[0]
        _header_markers = {"Source_Page_Number", "Volume_Number", "Major_Head_Code"}
        _is_header = bool(_header_markers & set(cell.strip() for cell in first_row))

        if _is_header:
            header = first_row
            if header != list(self.schema):
            ...
            # First row is data — prepend the schema header.
            header = list(self.schema)
            report.issues.append(
                Issue(
                    row_number=0,
                    column="HEADER",
                    message=f"No header row found; prepended schema header ({self.schema_name})",
                    code="HEADER_PREPENDED",
                    fixed=True,
                    severity="info",
                )
            )
        ...
        for row_number, raw_row in enumerate(data_rows, start=data_start):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="csv_cleaner",
        description='Cleanup CSV files generated from Karnataka budget extraction')
    parser.add_argument('-o', '--out-path',
                        default="OUT/15_sr_ka_exp",
                        help='Base output directory containing csv_outputs/')
    ...
    # Re-import get_schemas to use dynamic financial years if needed
    # but the current script has schemas at top level.
    # Actually, the refactored version of this script uses financial_columns in __init__.
    
    financial_columns = [x.strip() for x in args.financial_years.split(',')]
    if len(financial_columns) != 4:
    ...
    # Use provided path (could be absolute or relative to project root)
    ...
    for folder, schema_name, schema in CSV_TYPE_CONFIG:
        ...
        processor = CSVFileProcessor(schema_name, schema, financial_columns)
